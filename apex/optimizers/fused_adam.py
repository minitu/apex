    ...
    def __init__(self, params, lr=1e-3, bias_correction=True,
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            self.param_groups_master.append({
                'params': [
                    p.clone().detach().float() if self.master_weights and (p.dtype == torch.float16) else None
                    for p in param_list
                ],
            })
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=None, grad_norms=None, grad_scaler=None):
        ...
        for group, group_master in zip(self.param_groups, self.param_groups_master):
            ...
            if self.capturable:
                ...
                if len(g_16) > 0:
                    multi_tensor_applier(self.multi_tensor_adam_master if self.master_weights
                            else self.mult_tensor_adam,
                            self._dummy_overflow_buf,
                            [g_16, p_16, m_16, v_16, p16_master] if self.master_weights
                            else [g_16, p_16, m_16, v_16],
                            group['lr'],
                            beta1,
                            beta2,
